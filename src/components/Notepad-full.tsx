...
export default function Notepad({ onClose, onMinimize, isMaximized, onToggleMaximize }: NotepadProps) {
  ...
  // Safety check - ensure store has valid data
  useEffect(() => {
    if (!store || !store.items || Object.keys(store.items).length === 0) {
      console.warn('[Notepad] Store is empty or invalid');
    }
  }, [store]);
  
  // Early return if store is invalid
  if (!store || !store.items) {
    return (
      <div style={{ position: 'fixed', top: '10%', left: '10%', right: '10%', bottom: '10%', backgroundColor: '#252526', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8 }}>
        <X size={20} color="#e74c3c" />
        <span style={{ color: '#e74c3c', fontSize: 12 }}>记事本数据加载失败</span>
        <button onClick={onClose} style={{ padding: '4px 12px', borderRadius: 4, border: 'none', backgroundColor: '#dcb862', color: '#1e1e1e', fontSize: 11, cursor: 'pointer' }}>关闭</button>
      </div>
    );
  }
